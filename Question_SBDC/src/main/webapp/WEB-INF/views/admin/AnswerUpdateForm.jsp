		...
		<div class="row">
			...
			<div class="col-10">
				<form name="frm" method="post">
					...
					<table class="table table-striped table-sm">
						<thead>
							<tr>
								...
								<!-- 230526 (문항 실 번호) 에서 (문항 번호)로 수정 -->
								<th scope="col">문항 단계</th>
								<!-- 230526 (문항 실 단계) 에서 (문항 단계)로 수정 -->
								<th scope="col">문항 세부단계</th>
								<!-- 230526 (문항 세부단계 번호) 에서 (문항 세부단계)로 수정 -->
					...
					<!-- 230522 (하위 선택가능 답변) 에서 (해당문항 답변선택)으로 수정 -->
					<table class="table table-striped table-sm">
						<c:choose>
							...
							<c:otherwise>
								<table class="table table-striped table-sm">
									...
									<c:forEach var="avoList" items="${avoList}">
										<input type="hidden" name="answ_real_num"
											value="${avoList.answ_real_num}">
										<tr>
											<td>${avoList.answ_real_num}</td>
											<td>${avoList.answ_q_inside_num}</td>
											<td><input type="text" name="answ_next_level"
												value="${avoList.answ_next_level}"></td>
											<td><input type="text" name="answ_next_num"
												value="${avoList.answ_next_num}"></td>
											<td><input type="text" name="answ_text"
												value="${avoList.answ_text}"></td>
								...
								<button type="submit" class="btn btn-primary px-3"
									onclick="Answer_UpdateChk();">저장</button>
								<button type="reset" class="btn btn-outline-secondary"
									onclick="history.back()">뒤로가기</button>
								<!-- 230524 (취소) 에서 (뒤로가기)로 문구수정, onclick 추가 -->
								</form>
								<div class="col-1"></div>
								</div>
								</div>
