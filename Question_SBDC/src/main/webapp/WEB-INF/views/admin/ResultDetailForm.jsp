	...
	<div class="container-fluid" style = "height : 600px;">
		<div class="row">
			...
			<div class="col-10">
				<div>
				<form name="frm">
					<input type="hidden" name="delete" value="${rvo.biz_num}"
						checked="checked">
					<button type="button" class="btn btn-primary px-3"
						onclick="location.href='AdminMain?ListType=Result'">목록으로</button>
					<button type="button" class="btn btn-primary px-3"
						onclick="location.href='AdminResultUpdateForm?biz_num=${rvo.biz_num}'">사업수정</button>
					<button type="button" class="btn btn-primary px-3"
						onclick="del_Result();">사업삭제</button>
					<button type="button" class="btn btn-primary px-3"
						onclick="Popup_Memory('${rvo.biz_num}', '결과');">사업 이력관리</button>
					...
					<br> <input type="hidden" name="biz_num"
						value="${rvo.biz_num}">
					<table class="table table-striped table-sm">
						...
						<tr>
							<td>${rvo.biz_num}</td>
							<td>${rvo.res_first_writer}</td>
							<td>${rvo.res_biz_text}</td>
							<td>${rvo.res_link}</td>
							<td>${rvo.res_count}</td>
							<td>${rvo.res_date}</td>
